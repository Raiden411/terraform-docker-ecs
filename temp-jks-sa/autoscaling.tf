...
resource "aws_launch_configuration" "jks_on_demand" {
  ...
  security_groups             = ["${data.terraform_remote_state.common.sg_jks_sa_id}"]
...
resource "aws_autoscaling_group" "jks_on_demand" {
  name                      = "${var.name_prefix}_jks_on_demand"
  max_size                  = 1
  min_size                  = 1
  ...
  force_delete              = true
  launch_configuration      = "${aws_launch_configuration.jks_on_demand.name}"
  vpc_zone_identifier       = ["${data.terraform_remote_state.common.subnet_ids}"]

  tag {
    key                 = "Name"
    value               = "${var.name_prefix}_jks_on_demand"
